let hookStates = [];
let hookIndex = 0;

let effectList = [];
let effectIndex = 0;

let individualElement;
let individualContainer;

export function useCallback(callback, deps) {
  return useMemo(() => callback, deps);
}

// function updateCallback(callback, deps, individualCallbackIndex) {
//   const newDeps = deps === undefined ? null : deps;
//   const prevState = hookStates[individualCallbackIndex];

//   if (prevState) {
//     if (newDeps) {
//       let isEqual = true;
//       const prevDeps = prevState.newDeps;

//       for (let i = 0; i < prevDeps.length; i++) {
//         if (newDeps[i] !== prevDeps[i]) {
//           isEqual = false;
//           break;
//         }
//       }

//       if (isEqual) {
//         hookIndex++;
//         return prevState.newCallback;
//       }
//     }
//   }

//   hookStates[individualCallbackIndex] = {
//     newCallback: callback,
//     newDeps: deps,
//   };

//   hookIndex++;

//   return callback;
// }

// export function useCallback(callback, deps) {
//   const individualCallbackIndex = hookIndex;

//   if (hookStates[individualCallbackIndex]) {
//     const newCallback = updateCallback(callback, deps, individualCallbackIndex);
//     return newCallback;
//   }

//   const updateDeps = deps === undefined ? null : deps;

//   hookStates[individualCallbackIndex] = {
//     newCallback: callback,
//     newDeps: updateDeps,
//   };

//   hookIndex++;

//   return callback;
// }

function updateMemo(nextCreate, deps, individualMemoIndex) {
  const nextDeps = deps === undefined ? null : deps;
  const prevState = hookStates[individualMemoIndex];

  if (prevState) {
    if (nextDeps) {
      let isEqual = true;
      const prevDeps = prevState.deps;

      for (let i = 0; i < prevDeps.length; i++) {
        if (prevDeps[i] !== nextDeps[i]) {
          isEqual = false;
          break;
        }
      }

      if (isEqual) {
        hookIndex++;
        return prevState.value;
      }
    }
  }

  const newValue = nextCreate();

  hookStates[individualMemoIndex] = {
    value: newValue,
    deps: nextDeps,
  };

  hookIndex++;

  return newValue;
}

export function useMemo(nextCreate, deps) {
  const individualMemoIndex = hookIndex;

  if (hookStates[individualMemoIndex]) {
    const newValue = updateMemo(nextCreate, deps, individualMemoIndex);
    return newValue;
  }

  const newValue = nextCreate();
  const newDeps = deps === undefined ? null : deps;

  hookStates[individualMemoIndex] = {
    value: newValue,
    deps: newDeps,
  };

  hookIndex++;

  return newValue;
}

export function useRef(initialValue) {
  const individualRefIndex = hookIndex;

  if (hookStates[individualRefIndex]) {
    hookIndex++;
    return hookStates[individualRefIndex];
  }

  const ref = { current: initialValue };

  hookStates[individualRefIndex] = ref;

  hookIndex++;

  return hookStates[individualRefIndex];
}

function actionEffect() {
  for (let i = 0; i < effectList.length; i++) {
    if (!effectList[i].execute) {
      continue;
    }

    if (typeof effectList[i].cleanUp === "function") {
      effectList[i].cleanUp();
    }

    effectList[i].cleanUp = effectList[i].callback();
  }
}

export function useEffect(callback, deps) {
  const individualCallbackIndex = effectIndex;
  let setExecution = true;

  if (
    effectList[individualCallbackIndex] &&
    effectList[individualCallbackIndex].deps &&
    effectList[individualCallbackIndex].deps.length === 0
  ) {
    setExecution = false;
  }

  if (
    effectList[individualCallbackIndex] &&
    effectList[individualCallbackIndex].deps &&
    effectList[individualCallbackIndex].deps.length > 0
  ) {
    let hasDiff = false;
    for (let i = 0; i < effectList[individualCallbackIndex].deps.length; i++) {
      if (effectList[individualCallbackIndex].deps[i] !== deps[i]) {
        hasDiff = true;
      }
    }
    if (!hasDiff) {
      setExecution = false;
    }
  }

  effectList[individualCallbackIndex] = {
    callback: callback,
    deps: deps,
    cleanUp: effectList[individualCallbackIndex]
      ? effectList[individualCallbackIndex].cleanUp
      : undefined,
    execute: setExecution,
  };
  effectIndex++;
}

function rerender() {
  render(individualElement, individualContainer);
}

export function useState(props) {
  const individualIndex = hookIndex;

  if (hookStates[individualIndex] === undefined) {
    hookStates[individualIndex] = props;
  }

  const setState = (setStateProps) => {
    if (typeof setStateProps === "function") {
      const result = setStateProps(hookStates[individualIndex]);
      hookStates[individualIndex] = result;
    } else {
      hookStates[individualIndex] = setStateProps;
    }
    rerender();
  };

  hookIndex++;

  return [hookStates[individualIndex], setState];
}

export function createElement(type, props, ...children) {
  let updatedChildren = [...children];
  if (updatedChildren.length === 0) {
    updatedChildren = undefined;
  } else if (updatedChildren.length === 1) {
    updatedChildren = updatedChildren[0];
  }

  let updatedProps = { ...props, children: updatedChildren };

  return {
    type: type,
    props: updatedProps,
    key: null,
    ref: null,
  };
}

function renderFunction(element, container) {
  const functionResult = element.type(element.props);
  secondRender(functionResult, container);
}

function secondRender(element, container) {
  const outer = document.createElement(element.type);
  container.appendChild(outer);
  if (element.props.className) outer.className = element.props.className;

  const renderChildren = (outer, element) => {
    if (Array.isArray(element.props.children)) {
      for (const child of element.props.children) {
        if (typeof child.type === "function") {
          renderFunction(child, outer);
          continue;
        }

        const inner = document.createElement(child.type);

        outer.appendChild(inner);
        if (child.props.className) inner.className = child.props.className;

        if (
          typeof child.props.children === "string" ||
          typeof child.props.children === "number"
        ) {
          const textNode = document.createTextNode(child.props.children);
          inner.appendChild(textNode);
        } else if (Array.isArray(child.props.children)) {
          renderChildren(inner, child);
        } else if (typeof child.props.children === "object") {
          if (typeof child.props.children.type === "function") {
            renderFunction(child.props.children, inner);
            continue;
          }

          const grandInner = document.createElement(child.props.children.type);
          inner.appendChild(grandInner);
          if (child.props.children.props.className)
            grandInner.className = child.props.children.props.className;
          renderChildren(grandInner, child.props.children);
        }
      }
    } else if (
      typeof element.props.children === "string" ||
      typeof element.props.children === "number"
    ) {
      const textNode = document.createTextNode(element.props.children);
      outer.appendChild(textNode);
    } else if (typeof element.props.children === "object") {
      if (typeof element.props.children.type === "function") {
        renderFunction(element.props.children, outer);
        return;
      }

      const inner = document.createElement(element.props.children.type);
      outer.appendChild(inner);
      if (element.props.children.props.className)
        inner.className = element.props.children.props.className;

      renderChildren(inner, element.props.children);
    }
  };

  renderChildren(outer, element);
}

export function render(element, container) {
  if (individualContainer !== container) {
    hookStates = [];
    effectList = [];
  }

  individualElement = element;
  individualContainer = container;
  hookIndex = 0;

  effectIndex = 0;

  while (container.firstChild) {
    container.removeChild(container.firstChild);
  }

  if (typeof element.type === "function") {
    renderFunction(element, container);
    actionEffect();
    return;
  }

  secondRender(element, container);
  actionEffect();
}
